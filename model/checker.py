...
if __name__ == "__main__":
    ...
    # Load classifier
    clf = joblib.load(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "classifier.pkl")
    )
    features = pickle.loads(
        open(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "features.pkl"),
            "rb",
        ).read()
    )
    ...
    print(
        "The file %s is %s"
        % (os.path.basename(sys.argv[1]), ["malicious", "legitimate"][res])
    )
